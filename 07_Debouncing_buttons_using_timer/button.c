...
static uint64_t button_times[NUM_BUTTONS];
...
{
    int i, mask, button;
    uint64_t current_time, distance;
    ...
    for (i = 0; i < 8; i++)
    {
        if ((flags & mask) != 0)
        {
            ...
            if (button >= 0)
            {
                current_time = stimeElapsedMillis();
                distance = current_time - button_times[button];

                if (distance > BUTTON_DEBOUNCE_MS)
                {
                    buttonCallback(button);
                }

                button_times[button] = current_time;
            }
        }
        mask = mask << 1;
    }
...
/**
 @brief Initialization function with debouncing
 */
void buttonsInit(void)
{
    ...
    for (i = 0; i < NUM_BUTTONS; i++)
    {
        button_times[i]=stimeElapsedMillis(); //stimeGetEllapsedMs()
    ...
    else
    {
        if ((_pinrefs[which_button].port_is_odd) == 1)
        {
            ...
            else
            {
                ...
                current_time = stimeElapsedMillis();
                distance = current_time - button_times[which_button];
                ...
                button_times[which_button] = current_time;
            }
        }
        else
        {
